...
const REPORT_STATUSES = new Set(["Pending", "Acknowledged", "Resolved"]);
...
const INVENTORY_STATUSES = new Set(["OK", "Low", "Critical", "Out of Stock"]);

function enumValue(value, values, field, fallback) {
  const normalized = value ?? fallback;
  if (typeof normalized !== "string" || !values.has(normalized)) {
    throw new SyncError(400, "VALIDATION_ERROR", `${field} is invalid.`);
  }
  return normalized;
}
...
function text(value, field, max = 500) {
  ...
  if (normalized.length > max)
    throw new SyncError(400, "VALIDATION_ERROR", `${field} is too long.`);
  return normalized;
}
...
function validateProposalPayload(type, payload) {
  ...
  switch (type) {
    case "ADD_REPORT": {
      ...
      if (typeof location !== "object" || Array.isArray(location)) {
        throw new SyncError(
          400,
          "VALIDATION_ERROR",
          "Report location is invalid.",
        );
      }
      optionalNumber(location.lat, "Latitude", { min: -90, max: 90 });
      optionalNumber(location.lng, "Longitude", { min: -180, max: 180 });
      numberValue(payload.severity, "Severity", {
        min: 1,
        max: 5,
        integer: true,
      });
      enumValue(payload.status, REPORT_STATUSES, "Report status", "Pending");
      numberValue(payload.affectedCount ?? 0, "Affected count", {
        min: 0,
        integer: true,
      });
      numberValue(
        payload.peopleCount ?? payload.affectedCount ?? 0,
        "People count",
        { min: 0, integer: true },
      );
      numberValue(payload.daysWithoutFood ?? 0, "Days without food", {
        min: 0,
        integer: true,
      });
      numberValue(payload.waterLevelFt ?? 0, "Water level", { min: 0 });
      numberValue(payload.distanceFromAidKm ?? 0, "Distance from aid", {
        min: 0,
      });
      numberValue(payload.urgencyScore ?? 0, "Urgency score", {
        min: 0,
        max: 100,
        integer: true,
      });
    ...
    case "UPDATE_REPORT": {
      ...
      if (typeof patch !== "object" || Array.isArray(patch))
        throw new SyncError(
          400,
          "VALIDATION_ERROR",
          "Report changes are invalid.",
        );
      if (Object.hasOwn(patch, "severity"))
        numberValue(patch.severity, "Severity", {
          min: 1,
          max: 5,
          integer: true,
        });
      if (Object.hasOwn(patch, "status"))
        enumValue(patch.status, REPORT_STATUSES, "Report status");
      if (Object.hasOwn(patch, "affectedCount"))
        numberValue(patch.affectedCount, "Affected count", {
          min: 0,
          integer: true,
        });
      return;
    ...
    case "ADD_TASK":
      enumValue(payload.priority, TASK_PRIORITIES, "Task priority", "Medium");
      enumValue(payload.status, TASK_STATUSES, "Task status", "To Do");
      return;
    case "UPDATE_TASK": {
      const patch = payload.patch || {};
      if (typeof patch !== "object" || Array.isArray(patch))
      ...
      if (Object.hasOwn(patch, "priority"))
        enumValue(patch.priority, TASK_PRIORITIES, "Task priority");
      if (Object.hasOwn(patch, "status"))
        enumValue(patch.status, TASK_STATUSES, "Task status");
      return;
    }
    case "ADD_INVENTORY":
      numberValue(payload.qty, "Inventory quantity", { min: 0 });
      enumValue(payload.status, INVENTORY_STATUSES, "Inventory status", "OK");
      return;
    case "UPDATE_INVENTORY": {
      ...
      if (typeof patch !== "object" || Array.isArray(patch))
        throw new SyncError(
          400,
          "VALIDATION_ERROR",
          "Inventory changes are invalid.",
        );
      if (Object.hasOwn(patch, "qty"))
        numberValue(patch.qty, "Inventory quantity", { min: 0 });
      if (Object.hasOwn(patch, "status"))
        enumValue(patch.status, INVENTORY_STATUSES, "Inventory status");
      return;
    }
    case "UPDATE_ITEM_QTY":
      text(payload.itemId, "Inventory item", 100);
      numberValue(payload.delta, "Inventory change");
      return;
    default:
      return;
  }
...
function mapProposal(row) {
  return {
    ...
    conflictState: row.rejection_reason?.startsWith("Conflict:")
      ? "conflict"
      : "none",
...
async function requireChanged(result) {
  if (result.rowCount !== 1)
    throw new SyncError(
      404,
      "RECORD_NOT_FOUND",
      "The target record was not found.",
    );
}
...
export async function applyMutation(db, type, payload, actor) {
  ...
  switch (type) {
    case "ADD_REPORT": {
      ...
      const longitude = optionalNumber(location.lng, "Longitude", {
        min: -180,
        max: 180,
      });
      const severity = numberValue(payload.severity, "Severity", {
        min: 1,
        max: 5,
        integer: true,
      });
      ...
      const affectedCount = numberValue(
        payload.affectedCount ?? 0,
        "Affected count",
        { min: 0, integer: true },
      );
      const peopleCount = numberValue(
        payload.peopleCount ?? affectedCount,
        "People count",
        { min: 0, integer: true },
      );
      const daysWithoutFood = numberValue(
        payload.daysWithoutFood ?? 0,
        "Days without food",
        { min: 0, integer: true },
      );
      const waterLevelFt = numberValue(
        payload.waterLevelFt ?? 0,
        "Water level",
        { min: 0 },
      );
      const distanceFromAidKm = numberValue(
        payload.distanceFromAidKm ?? 0,
        "Distance from aid",
        { min: 0 },
      );
      const urgencyScore = numberValue(
        payload.urgencyScore ?? 0,
        "Urgency score",
        { min: 0, max: 100, integer: true },
      );
      const reportedAt = payload.time || new Date().toISOString();
      const referencePrefix = getReportReferencePrefix({
        ...payload,
        time: reportedAt,
      });
      ...
      await db.query(
        ...
        [
          ...
          distanceFromAidKm,
          urgencyScore,
          payload.urgencyZone || "green",
          JSON.stringify(payload.urgencyFactors || []),
    ...
    case "UPDATE_REPORT": {
      ...
      if (Object.hasOwn(patch, "status"))
        patch.status = enumValue(
          patch.status,
          REPORT_STATUSES,
          "Report status",
        );
      if (Object.hasOwn(patch, "affectedCount"))
        patch.affectedCount = numberValue(
          patch.affectedCount,
          "Affected count",
          { min: 0, integer: true },
        );
    ...
    case "ADD_REPORT_NOTE": {
      const note = {
        id: payload.note?.id || crypto.randomUUID(),
        authorId: actor.id,
        author: actor.name,
        text: text(payload.note?.text, "Note", 2000),
    ...
    case "ADD_TASK":
      await db.query(
        ...
        [
          text(payload.id, "Task ID", 100),
          text(payload.title, "Task title", 200),
          payload.description || "",
          enumValue(
            payload.priority,
            TASK_PRIORITIES,
    ...
    case "UPDATE_TASK": {
      ...
      if (Object.hasOwn(patch, "status"))
        patch.status = enumValue(patch.status, TASK_STATUSES, "Task status");
    ...
    case "ADD_INVENTORY":
      await db.query(
        ...
        [
          text(payload.id, "Inventory ID", 100),
          text(payload.name, "Item name", 200),
          text(payload.category, "Category", 100),
          numberValue(payload.qty, "Inventory quantity", { min: 0 }),
          text(payload.unit, "Unit", 50),
          enumValue(
            payload.status,
            INVENTORY_STATUSES,
            "Inventory status",
          ...
          text(payload.warehouseId, "Warehouse", 100),
    ...
    case "UPDATE_INVENTORY": {
      const patch = { ...payload.patch };
      if (Object.hasOwn(patch, "qty"))
        patch.qty = numberValue(patch.qty, "Inventory quantity", { min: 0 });
      if (Object.hasOwn(patch, "status"))
        patch.status = enumValue(
          patch.status,
          INVENTORY_STATUSES,
      ...
      const query = updateSql("inventory", "id", payload.id, patch, {
        name: "name",
        category: "category",
        qty: "quantity",
        unit: "unit",
    ...
    case "UPDATE_ITEM_QTY": {
      ...
      await db.query(
        `INSERT INTO stock_log (id, item_id, change_amount, reason, user_id, user_name)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [
          payload.logId || crypto.randomUUID(),
          text(payload.itemId, "Inventory item", 100),
          numberValue(payload.delta, "Inventory change"),
          payload.reason || "Adjustment",
          actor.id,
          actor.name,
        ],
      );
    ...
    case "ADD_STOCK_LOG":
      await insertStockLog(
        db,
        { ...payload, id: text(payload.id, "Stock log ID", 100) },
        text(payload.itemId, "Inventory item", 100),
        actor,
      );
      return;
    case "ADD_MAP_PIN":
      await db.query(
        `INSERT INTO map_pins (id, report_id, latitude, longitude, location, water_level_ft,
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
        [
          text(payload.id, "Pin ID", 100),
          payload.reportId || null,
          payload.lat ?? payload.location?.lat,
          payload.lng ?? payload.location?.lng,
          payload.locationName || payload.district || null,
          payload.waterLevelFt ?? null,
          payload.peopleCount ?? null,
          payload.childrenPresent ?? null,
        ],
      );
      return;
...
export class SyncService {
  ...
  async submit(input, actor) {
    ...
    validateProposalPayload(type, payload);
    const current = await this.authoritative.snapshot();
    const baseSnapshotSeq = Number(input.baseSnapshotSeq);
    if (
      !Number.isSafeInteger(baseSnapshotSeq) ||
      baseSnapshotSeq < 0 ||
      baseSnapshotSeq > current.snapshotSeq
    ) {
      throw new SyncError(
        409,
        "INVALID_BASE_SNAPSHOT",
        "Refresh the authoritative snapshot before submitting.",
      );
    }
    ...
    if (existing) {
      if (existing.user_id !== actor.id) {
        throw new SyncError(
          ...
          "Proposal ID is already in use.",
        );
      }
      return { proposal: mapProposal(existing), duplicate: true };
    }
  ...
  async transact(callback) {
    ...
    try {
      ...
      await client.query(
        "SELECT snapshot_seq FROM snapshot_meta WHERE singleton = TRUE FOR UPDATE",
      );
  ...
  decide(id, input, actor) {
    return this.transact(async (client) => {
      ...
      if (!proposal)
        throw new SyncError(404, "PROPOSAL_NOT_FOUND", "Proposal not found.");
      if (proposal.status !== "Pending")
        return { proposal: mapProposal(proposal), duplicate: true };
      ...
      if (earlier)
        throw new SyncError(
          409,
          "PROCESS_EARLIER_FIRST",
          `Process earlier proposal ${earlier} first.`,
        );
      ...
      if (conflict) {
        ...
        const rejected = await proposals.decide(id, {
          status: "Rejected",
          reason: `Conflict: ${conflict} was accepted first.`,
          actorId: actor.id,
          snapshotSeq: current.snapshotSeq,
        });
      ...
      const accepted = await proposals.decide(id, {
        status: "Accepted",
        actorId: actor.id,
        snapshotSeq: sequence.snapshotSeq,
      });
