...
export default function ItemFormModal({ item, isOpen, onClose, onSave }) {
  const [form, setForm] = useState(() => createForm(item));

  useEffect(() => {
    if (isOpen) setForm(createForm(item));
  }, [item, isOpen]);

  const setField = (field, value) => setForm((current) => ({ ...current, [field]: value }));
  const changeQuantity = (amount) => setField("qty", Math.max(0, Number(form.qty || 0) + amount));

  function handleSave() {
    if (!form.name.trim()) return;
    onSave({ ...form, name: form.name.trim(), qty: Math.max(0, Number(form.qty) || 0) });
  }
